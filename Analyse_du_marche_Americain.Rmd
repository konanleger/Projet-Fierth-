---
title: "Analyse Bivariée : Prix vs Marché des Produits de Thé (Catégorie Tea) - Marché Nord-Américain"
...
# Analyse du Tea sur le marché nord-américain
...
# Filtrage pour ne garder que la catégorie Tea et le marché nord-américain
gnpd_tea <- gnpd %>%
  filter(`Sub-Category` == "Tea", Market %in% c("USA", "Canada")) %>%
  filter(!is.na(`Price per 100 g/ml in Euros`))

# Filtrage des valeurs aberrantes
gnpd_north_american_market_tea <- gnpd_tea %>%
...
proportion_conservee <- nrow(gnpd_north_american_market_tea) / total_lignes
cat("Proportion des données conservées :", round(proportion_conservee * 100, 2), "%\n")
```

## 2. Analyse du thé du marché nord-américain

### 2.1 Résumé Statistique des Variables

```{r summary}
# Résumé statistique du prix
summary_prix <- summary(gnpd_north_american_market_tea$`Price per 100 g/ml in Euros`)
print("Résumé statistique du prix :")
print(summary_prix)

# Test de normalité de Shapiro-Wilk
shapiro_test <- shapiro.test(gnpd_north_american_market_tea$`Price per 100 g/ml in Euros`)
...
prix <- gnpd_north_american_market_tea$`Price per 100 g/ml in Euros`
prix <- na.omit(prix)
...
ggplot(gnpd_north_american_market_tea %>% 
         filter(!is.na(`Price per 100 g/ml in Euros`),
                `Price per 100 g/ml in Euros`<=borne_sup),
       aes(x = `Price per 100 g/ml in Euros`)) +
  geom_histogram(binwidth = 1, fill = "skyblue", color = "black") +
  labs(title = "Distribution du prix par 100 g/ml (Marché Nord-Américain)",
...
ggplot(gnpd_north_american_market_tea%>% 
  ...
  labs(title = "Boxplot du prix par 100 g/ml (Marché Nord-Américain)",
       y = "Prix (€ / 100 g/ml)",
       caption = "Source : Base Mintel") +
  theme_minimal()
...
```{r}
ggplot(gnpd_north_american_market_tea, aes(x = log1p(`Price per 100 g/ml in Euros`))) +
  geom_histogram(binwidth = 0.2, fill = "tomato", color = "black") +
  labs(title = "Distribution du log(prix) - Marché Nord-Américain", 
...
### Evolution du prix du thé sur le marché nord-américain au cours du temps
...
gnpd_north_american_market_tea <- gnpd_north_american_market_tea %>%
  mutate(Mois = floor_date(as.Date(`Date Published`), unit = "month"))

# Creation de la variable mois
prix_par_mois <- gnpd_north_american_market_tea %>%
  filter(!is.na(`Price per 100 g/ml in Euros`)) %>%
  group_by(Mois) %>%
...
ggplot(prix_par_mois, aes(x = Mois, y = Prix_moyen)) +
  ...
  labs(title = "Évolution du prix moyen du thé sur le marché nord-américain par mois",
       x = "Mois",
...
ggplot(prix_par_mois, aes(x = Mois, y = Prix_moyen)) +
  ...
  labs(title = "Tendance du prix du thé sur le marché nord-américain",
       x = "Mois", y = "Prix moyen (€)",
       caption = "Source : Base Mintel") +
  theme_minimal()
...
```{r}
prix_par_annee <- gnpd_north_american_market_tea %>%
...
ggplot(prix_par_annee, aes(x = annee, y = prix_moyen)) +
  ...
  labs(
    title = "Évolution des Prix Moyens sur le Marché Nord-Américain",
...
ggplot(prix_par_annee, aes(x = annee, y = prix_median)) +
  ...
  labs(
    title = "Évolution des Prix Médians sur le Marché Nord-Américain",
...
knitr::kable(prix_par_annee, 
             caption = "Statistiques des Prix par Année - Marché Nord-Américain")
```

# Analyse des type de produit lancés sur le marché nord-américain
...
launch_dist <- gnpd_north_american_market_tea %>%
...
ggplot(launch_dist, aes(x = reorder(`Launch Type`, n), y = n)) +
  ...
  coord_flip() +
  labs(
    title = "Répartition des Types de Lancement du Thé sur le Marché Nord-Américain",
    x = "Type de Lancement",
...
```{r}
launch_evolution <- gnpd_north_american_market_tea %>%
  mutate(annee = lubridate::year(`Date Published`)) %>%
  filter(!is.na(`Launch Type`)) %>%
...
ggplot(launch_evolution, aes(x = annee, y = n, color = `Launch Type`)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Évolution des Types de Lancement du Thé sur le Marché Nord-Américain",
    x = "Année",
...
ggplot(gnpd_north_american_market_tea, aes(x = `Launch Type`, y = `Price per 100 g/ml in Euros`)) +
  geom_boxplot(fill = "lightblue") +
  coord_flip() +
  labs(
    title = "Distribution du Prix par Type de Lancement - Marché Nord-Américain",
    x = "Type de Lancement",
...
```{r}
kruskal.test(`Price per 100 g/ml in Euros` ~ `Launch Type`, data = gnpd_north_american_market_tea)

```

# Interpretation

Le test de Kruskal-Wallis a été réalisé pour évaluer s'il existe une différence significative entre les prix du thé (€/100g ou 100ml) selon les différents types de lancement de produit (Launch Type) sur le marché nord-américain.
...
👉 Il existe donc à priori une différence statistiquement significative du prix moyen du thé selon le type de lancement sur le marché nord-américain
...
private_label_pie <- gnpd_north_american_market_tea %>%
...
ggplot(private_label_pie, aes(x = "", y = n, fill = `Private Label`)) +
  ...
  labs(
    ...
    et MDD sur le marché nord-américain",
    fill = "Type de marque",
    caption = "Source : Base Mintel"
  ) +
  theme_void() +
...
```{r}
gnpd_north_american_market_tea %>%
...
```{r}
ggplot(gnpd_north_american_market_tea, aes(x = `Private Label`, y = `Price per 100 g/ml in Euros`, fill = `Private Label`)) +
  geom_boxplot() +
  scale_y_continuous(limits = c(0, quantile(gnpd_north_american_market_tea$`Price per 100 g/ml in Euros`, 0.95, na.rm = TRUE))) +
  labs(
    title = "Distribution des prix selon le type de marque - Marché Nord-Américain",
    x = "Type de marque",
    y = "Prix (€/100g ou 100ml)",
    caption = "Source : Base Mintel"
  ) +
...
```{r}
wilcox.test(`Price per 100 g/ml in Euros` ~ `Private Label`, data = gnpd_north_american_market_tea)
...
Conclusion : Il existe une différence statistiquement significative entre les prix des produits "Branded" et "Private Label" sur le marché nord-américain.

## Construction de modele d'un premier modele 

```{r}
lm_model <- lm(`Price per 100 g/ml in Euros` ~ `Private Label`, data = gnpd_north_american_market_tea)
summary(lm_model)
```

## analyse de la qualité du thé sur marché nord-américain

```{r}
# Extraction des claims individuels
claims <- gnpd_north_american_market_tea %>%
...
claims %>%
  ...
  coord_flip() +
  labs(title = "Principaux arguments marketing sur le marché nord-américain", 
       x = "Claim", y = "Fréquence",
       caption = "Source : Base Mintel")
...
claims_table <- gnpd_north_american_market_tea %>%
...
```
```{r}
ggplot(gnpd_north_american_market_tea, aes(x = factor(niveau_qualite), y = `Price per 100 g/ml in Euros`)) +
  geom_boxplot(fill = "#69b3a2", alpha = 0.7) +
  #geom_jitter(width = 0.2, alpha = 0.3, color = "grey30") +
  labs(
    title = "Relation entre Score de Qualité et Prix - Marché Nord-Américain",
    x = "Score de Qualité ",
    y = "Prix (€/100g ou 100ml)",
    caption = "Source : Base Mintel"
  ) +
...
```{r}
modele_score <- lm(`Price per 100 g/ml in Euros` ~ niveau_qualite, data = gnpd_north_american_market_tea)
